...
**/
	int insert = 1; 										// inform me if i have the item with Content
	
	tBTNodePtr newItm = malloc(sizeof(struct tBTNode));		// create new  itm
	if ( newItm == NULL ) {
			exit(99);
	}
	...
	if ( (*RootPtr) == NULL ) {
	...
	else {													// if tree inst creater it has no sence to give there itm
			tBTNodePtr current = (*RootPtr);
			tBTNodePtr parrent = NULL;
			...
			while ( current != NULL ) {
					...
					else if ( Content > current->Cont ) {
							current = current->RPtr;
					}

					else if ( current->Cont == Content ) {
								free(newItm);				// i dont insert item to BST so i free it
								insert = 0;
								break;
					}
			} // while
			if ( insert ) {  								// i dont have node with this itm
					if ( Content < parrent->Cont ) {
							parrent->LPtr = newItm;
				}
				else {
						parrent->RPtr = newItm;
...
**/
	...
	Leftmost_Postorder(RootPtr, &stack, &stackBool);

	while ( !SEmptyP(&stack) ) {

			RootPtr = STopPopP(&stack);
			SPushP(&stack, RootPtr);
			left = STopPopB(&stackBool);
		
			if ( left ) {

					SPushB(&stackBool, false);
					Leftmost_Postorder(RootPtr->RPtr, &stack, &stackBool);
...
**/
	tStackP stack;	// stack for saving items
	SInitP(&stack);
